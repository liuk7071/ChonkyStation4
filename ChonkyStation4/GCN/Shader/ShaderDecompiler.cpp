...
struct DecompilerState {
    ...
    bool has_cubemap_id_func = false;
    bool has_cubemap_scoord_func = false;
    bool has_cubemap_tcoord_func = false;
    bool has_cubemap_majoraxis_func = false;
...
uint mbcntHi(uint src, uint addend) {
    ...
    std::string emit(BasicBlock* from, BasicBlock* to, bool& needs_barrier, int level = 0, bool indent = false, bool is_in_loop = false, BasicBlock* loop_header = nullptr, bool ignore_loop_header = false) {
        ...
        if (from != to) {
            ...
            auto loop_check_emit = [this](BasicBlock* from, BasicBlock* to, bool& needs_barrier, int level = 0, bool indent = false, bool is_in_loop = false, BasicBlock* loop_header = nullptr) -> std::string {
                if (is_in_loop) {
                    if (from == loop_header) {
    ...
    void decompileBasicBlock(u32* data, u32 start_pc, ShaderStage stage, BasicBlock& block) {
        ...
        while (!code_slice.atEnd() && !done) {
            ...
            case Shader::Opcode::TBUFFER_LOAD_FORMAT_XYZW: {
                ...
                // Swizzle
                // TODO: I think TBUFFER uses the V# swizzle too? But it breaks TressFX in games...
                switch (instr.opcode) {
                case Shader::Opcode::BUFFER_LOAD_FORMAT_X:
                case Shader::Opcode::BUFFER_LOAD_FORMAT_XY:
                case Shader::Opcode::BUFFER_LOAD_FORMAT_XYZ:
                case Shader::Opcode::BUFFER_LOAD_FORMAT_XYZW: {
                    code += std::format("itmp = ivec4({}, {}, {}, {});\n", getVGPR(instr.src[1].code + 0), getVGPR(instr.src[1].code + 1), getVGPR(instr.src[1].code + 2), getVGPR(instr.src[1].code + 3));

                    auto* vsharp = buf->desc_info.asPtr<VSharp>(regs);
                    std::array<u32, 4> swizzles = { vsharp->dst_sel_x, vsharp->dst_sel_y, vsharp->dst_sel_z, vsharp->dst_sel_w };
                    for (int i = 0; i < instr.control.mtbuf.count; i++) {

                        auto swizzle = swizzles[i];
                        std::string val;
                        if (swizzle == DSEL_0)      val = "0";
                        else if (swizzle == DSEL_1) val = "f2u(1.0f)";
                        else if (swizzle == DSEL_R) val = "itmp.x";
                        else if (swizzle == DSEL_G) val = "itmp.y";
                        else if (swizzle == DSEL_B) val = "itmp.z";
                        else if (swizzle == DSEL_A) val = "itmp.w";

                        code += std::format("{} = {};\n", getVGPR(instr.src[1].code + i), val);
                    }
                    break;
                }
                }
...
template<typename T>
T* DescriptorLocation::asPtr(u32* regs) {
    if (regs == nullptr) regs = GCN::renderer->regs;
    ...
    if (is_ptr) {
        ...
        if (ptr_is_from_buf) {
            VSharp* vsharp = buf->desc_info.asPtr<VSharp>(regs);
        ...
        std::memcpy(&desc, &regs[base + sgpr], sizeof(T*));
        desc = (T*)((u32*)desc + offs);  // The immediate is an offset in dwords
        return desc;
    }
    else {
        return (T*)&regs[base + sgpr];
    }
}
template VSharp* DescriptorLocation::asPtr<VSharp>(u32* regs);
...
void decompileShader(u32* data, ShaderStage stage, ShaderData& out_data, FetchShader* fetch_shader, ComputeJob* compute_job, u32* regs) {
    ...
    DecompilerState state;
    state.shader.reserve(4_MB);

    state.regs = !regs ? GCN::renderer->regs : regs;
...
bool v_cmp_class_f32(float x, uint mask) {
    ...
    if (stage == ShaderStage::Compute) {
        state.shader += std::format("layout(local_size_x = {}, local_size_y = {}, local_size_z = {}) in;\n\n", compute_job->n_threads_x, compute_job->n_threads_y, compute_job->n_threads_z);
    }
    ...
    case ShaderStage::Vertex: {
        ...
        for (auto& binding : fetch_shader->bindings) {
            ...
            for (int i = 0; i < binding.n_elements; i++) {
                ...
                main += std::format("{} = {};\n", state.getVGPR(binding.dest_vgpr + i), val);
            }
        }
        break;
    }
    ...
    state.discoverBasicBlockEntries(data, 0);

    std::unordered_map<u32, DescriptorLocation> descs;
    std::unordered_map<u32, DescriptorLocation> backup_descs;
    state.trackAndCreateBuffers(data, 0, stage, out_data, descs, backup_descs);

    BasicBlock* start = state.blocks.emplace_back(std::make_unique<BasicBlock>()).get();
    start->is_start = true;
    state.decompileBasicBlock(data, 0, stage, *start);
    ...
    bool needs_barrier = false;
    main += state.emit(start, nullptr, needs_barrier, 0);

    state.shader += "\n";
    state.shader += state.const_tables;
    state.shader += "\n";
    ...
    if (state.need_get_vgpr_helper)
        state.addGetVGPRHelper();
    ...
    state.addFunc("void main", main);

    //printf("%s\n", shader.c_str());
    out_data.source = state.shader;
    out_data.has_gds = state.has_gds;

    if (stage == ShaderStage::Vertex) {
        // Save vertex outputs
        out_data.vtx_outputs.reserve(32);
        for (auto& [location, name] : state.out_attrs)
